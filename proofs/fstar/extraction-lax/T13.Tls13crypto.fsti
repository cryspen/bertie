...
unfold
let t_AeadIV = t13.Tls13utils.t_Bytes

unfold
let t_Digest = t13.Tls13utils.t_Bytes

unfold
let t_Hmac = t13.Tls13utils.t_Bytes

unfold
let t_KemPk = t13.Tls13utils.t_Bytes

unfold
let t_KemSk = t13.Tls13utils.t_Bytes

unfold
let t_Key = t13.Tls13utils.t_Bytes

unfold
let t_MacKey = t13.Tls13utils.t_Bytes

unfold
let t_Psk = t13.Tls13utils.t_Bytes

unfold
let t_Random = t13.Tls13utils.t_Bytes

unfold
let t_SignatureKey = t13.Tls13utils.t_Bytes

unfold
let t_VerificationKey = t13.Tls13utils.t_Bytes
...
val hkdf_extract (alg: t_HashAlgorithm) (ikm salt: t13.Tls13utils.t_Bytes)
    : Prims.Pure (Core.Result.t_Result t13.Tls13utils.t_Bytes u8)
      Prims.l_True
      (fun _ -> Prims.l_True)

val hmac_tag (alg: t_HashAlgorithm) (mk input: t13.Tls13utils.t_Bytes)
    : Prims.Pure (Core.Result.t_Result t13.Tls13utils.t_Bytes u8)
      Prims.l_True
      (fun _ -> Prims.l_True)
...
val sign
      ...
      (sk input: t13.Tls13utils.t_Bytes)
      (rng: impl_916461611_)
...
(*
val supported_rsa_key_size (n: t13.Tls13utils.t_Bytes)
...
val sign_rsa
      ...
      {| i1: Rand_core.t_CryptoRng impl_916461611_ |}
      {| i2: Rand_core.t_RngCore impl_916461611_ |}
      (sk pk_modulus pk_exponent: t13.Tls13utils.t_Bytes)
      (cert_scheme: t_SignatureScheme)
      (input: t13.Tls13utils.t_Bytes)
      (rng: impl_916461611_)
    : Prims.Pure (impl_916461611_ & Core.Result.t_Result t13.Tls13utils.t_Bytes u8)
      Prims.l_True
      (fun _ -> Prims.l_True)
...
val to_shared_secret (alg: t_KemScheme) (shared_secret: t13.Tls13utils.t_Bytes)
    : Prims.Pure t13.Tls13utils.t_Bytes Prims.l_True (fun _ -> Prims.l_True)

val kem_decap (alg: t_KemScheme) (ct sk: t13.Tls13utils.t_Bytes)
    : Prims.Pure (Core.Result.t_Result t13.Tls13utils.t_Bytes u8)
      Prims.l_True
      (fun _ -> Prims.l_True)
...
let impl_7: Core.Convert.t_TryFrom t_Algorithms string =
  {
    f_Error = t13.Tls13utils.t_Error;
    f_try_from_pre = (fun (s: string) -> true);
    f_try_from_post
    =
    (fun (s: string) (out: Core.Result.t_Result t_Algorithms t13.Tls13utils.t_Error) -> true);
    f_try_from
    =
    fun (s: string) ->
      match s with
      | "SHA256_Chacha20Poly1305_RsaPssRsaSha256_X25519" ->
        ...
        <:
        Core.Result.t_Result t_Algorithms t13.Tls13utils.t_Error
      ...
      | _ ->
        ...
        (t13.Tls13utils.Error_UnknownCiphersuite res <: t13.Tls13utils.t_Error)
        <:
        Core.Result.t_Result t_Algorithms t13.Tls13utils.t_Error
  }

type t_AeadKey = {
  f_bytes:t13.Tls13utils.t_Bytes;
...
*)
val impl__AeadKey__new (bytes: t13.Tls13utils.t_Bytes) (alg: t_AeadAlgorithm)
    : Prims.Pure t_AeadKey Prims.l_True (fun _ -> Prims.l_True)

type t_RsaVerificationKey = {
  f_modulus:t13.Tls13utils.t_Bytes;
  f_exponent:t13.Tls13utils.t_Bytes
}

type t_PublicVerificationKey =
  | PublicVerificationKey_EcDsa : t13.Tls13utils.t_Bytes -> t_PublicVerificationKey
  | PublicVerificationKey_Rsa : t_RsaVerificationKey -> t_PublicVerificationKey

val aead_decrypt (k: t_AeadKey) (iv cip aad: t13.Tls13utils.t_Bytes)
    : Prims.Pure (Core.Result.t_Result t13.Tls13utils.t_Bytes u8)
      Prims.l_True
      (fun _ -> Prims.l_True)
...
val verify
      ...
      (input sig: t13.Tls13utils.t_Bytes)
    : Prims.Pure (Core.Result.t_Result Prims.unit u8) Prims.l_True (fun _ -> Prims.l_True)
