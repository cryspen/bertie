...
let derive_iv_ctr (iv: Bertie.Tls13utils.t_Bytes) (n: u64) =
  ...
  in
  let iv_t = x:Bertie.Tls13utils.t_Bytes{Seq.length x._0 == Seq.length iv._0} in
  let iv_ctr:iv_t =
  ...
  let iv_ctr:iv_t =
    Core.Iter.Traits.Iterator.f_fold (Core.Iter.Traits.Collect.f_into_iter ({
      ...
      (fun iv_ctr i ->
          ...
          assert (Bertie.Tls13utils.impl__Bytes__len iv_ctr == Bertie.Tls13utils.impl__Bytes__len iv);
          let i:usize = i in
    ...
    | Core.Result.Result_Ok _ ->
      ...
      (match Bertie.Tls13crypto.aead_decrypt kiv.Bertie.Tls13crypto.f_key iv_ctr cip ad with
        | Core.Result.Result_Ok plain ->
          assume ((padlen plain (Bertie.Tls13utils.impl__Bytes__len plain <: usize) <: usize) <.
