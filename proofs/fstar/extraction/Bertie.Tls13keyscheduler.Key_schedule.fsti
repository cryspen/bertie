...
class t_KeySchedule (v_Self: Type0) (v_N: Type0) = {
  ...
  f_set:
      ...
      x1: v_N ->
      x2: u8 ->
      x3: (v_N & Bertie.Tls13crypto.t_HashAlgorithm & u8) ->
      x4: Bertie.Tls13utils.t_Bytes
    -> Prims.Pure v_Self (f_set_pre x0 x1 x2 x3 x4) (fun result -> f_set_post x0 x1 x2 x3 x4 result);
  f_hash_pre:Bertie.Tls13utils.t_Bytes -> Type0;
  f_hash_post:Bertie.Tls13utils.t_Bytes -> Bertie.Tls13utils.t_Bytes -> Type0;
  f_hash:x0: Bertie.Tls13utils.t_Bytes
    -> Prims.Pure Bertie.Tls13utils.t_Bytes (f_hash_pre x0) (fun result -> f_hash_post x0 result)
}
...
type t_TLSkeyscheduler = {
  f_keys:Std.Collections.Hash.Map.t_HashMap (t_TLSnames & Bertie.Tls13crypto.t_HashAlgorithm & u8)
    Bertie.Tls13utils.t_Bytes
...
type t_TagKey = {
  f_alg:Bertie.Tls13crypto.t_HashAlgorithm;
  f_tag:t_TLSnames;
...
val zero_salt (ks: t_TLSkeyscheduler) (alg: Bertie.Tls13crypto.t_HashAlgorithm)
    : Prims.Pure (t_TLSkeyscheduler & t_Handle) Prims.l_True (fun _ -> Prims.l_True)

/// Get an empty key of the correct size.
val no_psk (ks: t_TLSkeyscheduler) (alg: Bertie.Tls13crypto.t_HashAlgorithm)
    : Prims.Pure (t_TLSkeyscheduler & t_Handle) Prims.l_True (fun _ -> Prims.l_True)

/// Get an empty key of the correct size.
val zero_ikm (ks: t_TLSkeyscheduler) (alg: Bertie.Tls13crypto.t_HashAlgorithm)
    : Prims.Pure (t_TLSkeyscheduler & t_Handle) Prims.l_True (fun _ -> Prims.l_True)
