...
[@@ FStar.Tactics.Typeclasses.tcinstance]
let impl_22: Core.Convert.t_From t_AppData (t_Slice u8) =
  { f_from = fun (value: t_Slice u8) -> AppData (Core.Convert.f_into value) <: t_AppData }

[@@ FStar.Tactics.Typeclasses.tcinstance]
let impl_23 (v_N: usize) : Core.Convert.t_From t_AppData (t_Array u8 v_N) =
  { f_from = fun (value: t_Array u8 v_N) -> AppData (Core.Convert.f_into value) <: t_AppData }

[@@ FStar.Tactics.Typeclasses.tcinstance]
let impl_24: Core.Convert.t_From t_AppData (Alloc.Vec.t_Vec u8 Alloc.Alloc.t_Global) =
  {
    f_from
    =
...
[@@ FStar.Tactics.Typeclasses.tcinstance]
let impl_25: Core.Convert.t_From t_AppData t_Bytes =
  { f_from = fun (value: t_Bytes) -> AppData value <: t_AppData }

type t_HandshakeData = | HandshakeData : t_Bytes -> t_HandshakeData

let impl__HandshakeData__len (self: t_HandshakeData) : usize = impl__Bytes__len self._0

let impl__HandshakeData__to_bytes (self: t_HandshakeData) : t_Bytes = Core.Clone.f_clone self._0

let impl__HandshakeData__concat (self other: t_HandshakeData) : t_HandshakeData =
  let message1:t_Bytes = impl__HandshakeData__to_bytes self in
  let message2:t_Bytes = impl__HandshakeData__to_bytes other in
  let message1:t_Bytes =
    {
      message1 with
      _0
      =
      Alloc.Vec.impl_2__extend_from_slice message1._0
        (Core.Ops.Deref.f_deref message2._0 <: t_Slice t_U8)
    }
    <:
    t_Bytes
  in
...
[@@ FStar.Tactics.Typeclasses.tcinstance]
let impl_20: Core.Convert.t_From t_HandshakeData t_Bytes =
  { f_from = fun (value: t_Bytes) -> HandshakeData value <: t_HandshakeData }
