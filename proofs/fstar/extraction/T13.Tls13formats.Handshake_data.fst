...
let get_hs_type (t: u8) =
  ...
  | Rust_primitives.Integers.MkInt 254 ->
    ...
    <:
    Core.Result.t_Result t_HandshakeType u8
  | _ -> t13.Tls13utils.tlserr #t_HandshakeType (t13.Tls13utils.parse_failed () <: u8)

let to_bytes_inner (hs: t_HandshakeData) =
  Core.Clone.f_clone #t13.Tls13utils.t_Bytes #FStar.Tactics.Typeclasses.solve hs._0

let impl_HandshakeData__to_bytes (self: t_HandshakeData) = to_bytes_inner self

let impl_HandshakeData__len (self: t_HandshakeData) = t13.Tls13utils.impl_Bytes__len self._0

let impl_HandshakeData__next_handshake_message (self: t_HandshakeData) =
  if (impl_HandshakeData__len self <: usize) <. mk_usize 4
  then
    t13.Tls13utils.tlserr #(t_HandshakeData & t_HandshakeData)
      (t13.Tls13utils.parse_failed () <: u8)
  else
    match
      t13.Tls13utils.length_u24_encoded (t13.Tls13utils.impl_Bytes__raw_slice self._0
            ({
                Core.Ops.Range.f_start = mk_usize 1;
                Core.Ops.Range.f_end = t13.Tls13utils.impl_Bytes__len self._0 <: usize
              }
              <:
    ...
    | Core.Result.Result_Ok len ->
      let message:t13.Tls13utils.t_Bytes =
        t13.Tls13utils.impl_Bytes__slice_range self._0
          ({
...
let to_two_inner (hs_data: t_HandshakeData) =
  match
    impl_HandshakeData__next_handshake_message hs_data
    <:
    Core.Result.t_Result (t_HandshakeData & t_HandshakeData) u8
  with
...
let to_four_inner (hs_data: t_HandshakeData) =
  match
    impl_HandshakeData__next_handshake_message hs_data
    <:
    Core.Result.t_Result (t_HandshakeData & t_HandshakeData) u8
          ...
          | Core.Result.Result_Ok (message3, payload_rest) ->
            (match
              ...
              | Core.Result.Result_Ok (message4, payload_rest) ->
                if (impl_HandshakeData__len payload_rest <: usize) <>. mk_usize 0
                then
                  t13.Tls13utils.tlserr #(t_HandshakeData & t_HandshakeData & t_HandshakeData &
                      t_HandshakeData)
                    (t13.Tls13utils.parse_failed () <: u8)
                else
                  Core.Result.Result_Ok
...
let impl_HandshakeData__as_handshake_message
  ...
  | Core.Result.Result_Ok (message, payload_rest) ->
    if (impl_HandshakeData__len payload_rest <: usize) <>. mk_usize 0
    then t13.Tls13utils.tlserr #t_HandshakeData (t13.Tls13utils.parse_failed () <: u8)
    else
      let HandshakeData tagged_message_bytes:t_HandshakeData = message in
      (match
          t13.Tls13utils.check_eq1 (t13.Tls13utils.v_U8 (t_HandshakeType_cast_to_repr expected_type
        ...
        | Core.Result.Result_Ok _ ->
          Core.Result.Result_Ok
          (HandshakeData
            (t13.Tls13utils.impl_Bytes__slice_range tagged_message_bytes
                ({
                    Core.Ops.Range.f_start = mk_usize 4;
                    Core.Ops.Range.f_end
                    =
                    t13.Tls13utils.impl_Bytes__len tagged_message_bytes <: usize
                  }
                  <:
                  Core.Ops.Range.t_Range usize))
            <:
...
let impl_HandshakeData__to_two (self: t_HandshakeData) = to_two_inner self

let impl_HandshakeData__to_four (self: t_HandshakeData) = to_four_inner self

[@@ FStar.Tactics.Typeclasses.tcinstance]
let impl_6: Core.Convert.t_From t_HandshakeData t13.Tls13utils.t_Bytes =
  {
    f_from_pre = (fun (value: t13.Tls13utils.t_Bytes) -> true);
    f_from_post
    =
    (fun (value: t13.Tls13utils.t_Bytes) (result: t_HandshakeData) -> result._0 = value);
    f_from = fun (value: t13.Tls13utils.t_Bytes) -> HandshakeData value <: t_HandshakeData
  }

let from_bytes_inner (handshake_type: t_HandshakeType) (handshake_bytes: t13.Tls13utils.t_Bytes) =
  match
    t13.Tls13utils.encode_length_u24 handshake_bytes
    <:
  ...
  | Core.Result.Result_Ok hoist1 ->
    ...
    (Core.Convert.f_from #t_HandshakeData
        ...
        (t13.Tls13utils.impl_Bytes__prefix hoist1
            ((let list =
...
let impl_HandshakeData__concat (self other: t_HandshakeData) =
  let message1:t13.Tls13utils.t_Bytes = impl_HandshakeData__to_bytes self in
  let message2:t13.Tls13utils.t_Bytes = impl_HandshakeData__to_bytes other in
  let message1:t13.Tls13utils.t_Bytes =
    t13.Tls13utils.impl_Bytes__extend_from_slice message1 message2
  in
  Core.Convert.f_from #t_HandshakeData
    #t13.Tls13utils.t_Bytes
    #FStar.Tactics.Typeclasses.solve
