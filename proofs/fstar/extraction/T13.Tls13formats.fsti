...
let build_server_name__v_PREFIX1: t_Array u8 (mk_usize 2) =
  let list = [t13.Tls13utils.v_U8 (mk_u8 0); t13.Tls13utils.v_U8 (mk_u8 0)] in
  FStar.Pervasives.assert_norm (Prims.eq2 (List.Tot.length list) 2);
  Rust_primitives.Hax.array_of_list 2 list

let build_server_name__v_PREFIX2: t_Array u8 (mk_usize 1) =
  let list = [t13.Tls13utils.v_U8 (mk_u8 0)] in
...
val build_server_name (name: t13.Tls13utils.t_Bytes)
    : Prims.Pure (Core.Result.t_Result t13.Tls13utils.t_Bytes u8)
      ...
      (ensures
        fun result ->
          let result:Core.Result.t_Result t13.Tls13utils.t_Bytes u8 = result in
          match result <: Core.Result.t_Result t13.Tls13utils.t_Bytes u8 with
          | Core.Result.Result_Ok b ->
            (t13.Tls13utils.impl_Bytes__len name <: usize) <. mk_usize 65536 &&
            (t13.Tls13utils.impl_Bytes__len b <: usize) =.
            ((t13.Tls13utils.impl_Bytes__len name <: usize) +! mk_usize 9 <: usize)
          | _ -> true)
  ...
  -> Prims.Pure (Core.Result.t_Result t13.Tls13utils.t_Bytes u8)
      ...
      (ensures
        fun result ->
          ...
          match result <: Core.Result.t_Result t13.Tls13utils.t_Bytes u8 with
          | Core.Result.Result_Ok b ->
            (t13.Tls13utils.impl_Bytes__len b <: usize) <=. mk_usize 260
          | _ -> true)
...
let server_supported_version__v_SUPPORTED_VERSION_PREFIX: t_Array u8 (mk_usize 2) =
  let list = [t13.Tls13utils.v_U8 (mk_u8 0); t13.Tls13utils.v_U8 (mk_u8 43)] in
  FStar.Pervasives.assert_norm (Prims.eq2 (List.Tot.length list) 2);
  Rust_primitives.Hax.array_of_list 2 list

val server_supported_version (e_algorithms: t13.Tls13crypto.t_Algorithms)
    : Prims.Pure (Core.Result.t_Result t13.Tls13utils.t_Bytes u8)
      Prims.l_True
      (fun _ -> Prims.l_True)

val check_server_supported_version (e_algs: t13.Tls13crypto.t_Algorithms) (b: t_Slice u8)
    : Prims.Pure (Core.Result.t_Result Prims.unit u8) Prims.l_True (fun _ -> Prims.l_True)

let supported_groups__v_SUPPORTED_GROUPS_PREFIX: t_Array u8 (mk_usize 2) =
  let list = [t13.Tls13utils.v_U8 (mk_u8 0); t13.Tls13utils.v_U8 (mk_u8 10)] in
  FStar.Pervasives.assert_norm (Prims.eq2 (List.Tot.length list) 2);
  Rust_primitives.Hax.array_of_list 2 list

val supported_groups (algs: t13.Tls13crypto.t_Algorithms)
    : Prims.Pure (Core.Result.t_Result t13.Tls13utils.t_Bytes u8)
      Prims.l_True
      (fun _ -> Prims.l_True)

val check_supported_groups (algs: t13.Tls13crypto.t_Algorithms) (ch: t_Slice u8)
    : Prims.Pure (Core.Result.t_Result Prims.unit u8) Prims.l_True (fun _ -> Prims.l_True)

val signature_algorithms (algs: t13.Tls13crypto.t_Algorithms)
    : Prims.Pure (Core.Result.t_Result t13.Tls13utils.t_Bytes u8)
      Prims.l_True
      (fun _ -> Prims.l_True)
...
let psk_key_exchange_modes__v_PSK_MODE_PREFIX: t_Array u8 (mk_usize 2) =
  let list = [t13.Tls13utils.v_U8 (mk_u8 0); t13.Tls13utils.v_U8 (mk_u8 45)] in
  FStar.Pervasives.assert_norm (Prims.eq2 (List.Tot.length list) 2);
  Rust_primitives.Hax.array_of_list 2 list
...
let key_shares__v_PREFIX: t_Array u8 (mk_usize 2) =
  let list = [t13.Tls13utils.v_U8 (mk_u8 0); t13.Tls13utils.v_U8 (mk_u8 51)] in
  FStar.Pervasives.assert_norm (Prims.eq2 (List.Tot.length list) 2);
  Rust_primitives.Hax.array_of_list 2 list
...
val pre_shared_key
    ...
    : Prims.Pure (Core.Result.t_Result (t13.Tls13utils.t_Bytes & usize) u8)
      Prims.l_True
      (fun _ -> Prims.l_True)

val check_psk_shared_key (algs: t13.Tls13crypto.t_Algorithms) (ch: t_Slice u8)
    : Prims.Pure (Core.Result.t_Result (t13.Tls13utils.t_Bytes & t13.Tls13utils.t_Bytes) u8)
      Prims.l_True
      (fun _ -> Prims.l_True)

val server_pre_shared_key (e_algs: t13.Tls13crypto.t_Algorithms)
    : Prims.Pure (Core.Result.t_Result t13.Tls13utils.t_Bytes u8)
      Prims.l_True
      (fun _ -> Prims.l_True)

val check_server_psk_shared_key (e_algs: t13.Tls13crypto.t_Algorithms) (b: t_Slice u8)
    : Prims.Pure (Core.Result.t_Result Prims.unit u8) Prims.l_True (fun _ -> Prims.l_True)

/// TLS Extensions
type t_Extensions = {
  f_sni:Core.Option.t_Option t13.Tls13utils.t_Bytes;
  f_key_share:Core.Option.t_Option t13.Tls13utils.t_Bytes;
  f_ticket:Core.Option.t_Option t13.Tls13utils.t_Bytes;
  f_binder:Core.Option.t_Option t13.Tls13utils.t_Bytes
}
...
val check_server_extension (algs: t13.Tls13crypto.t_Algorithms) (b: t_Slice u8)
    : Prims.Pure (Core.Result.t_Result (usize & Core.Option.t_Option t13.Tls13utils.t_Bytes) u8)
      ...
      (ensures
        fun result ->
          let result:Core.Result.t_Result (usize & Core.Option.t_Option t13.Tls13utils.t_Bytes)
            u8 =
            result
          in
          match
...
let impl_1: Core.Convert.t_TryFrom t_AlertLevel u8 =
  {
    ...
    fun (value: u8) ->
      ...
      | Rust_primitives.Integers.MkInt 2 ->
        ...
        <:
        Core.Result.t_Result t_AlertLevel u8
      | _ -> t13.Tls13utils.tlserr #t_AlertLevel (t13.Tls13utils.parse_failed () <: u8)
  }
...
let impl_3: Core.Convert.t_TryFrom t_AlertDescription u8 =
  {
    ...
    fun (value: u8) ->
      ...
      | Rust_primitives.Integers.MkInt 120 ->
        ...
        <:
        Core.Result.t_Result t_AlertDescription u8
      | _ -> t13.Tls13utils.tlserr #t_AlertDescription (t13.Tls13utils.parse_failed () <: u8)
  }
...
val get_psk_extensions
    ...
    : Prims.Pure (Core.Result.t_Result (usize & t13.Tls13utils.t_Bytes) u8)
      ...
      (ensures
        fun result ->
          let result:Core.Result.t_Result (usize & t13.Tls13utils.t_Bytes) u8 = result in
          match result <: Core.Result.t_Result (usize & t13.Tls13utils.t_Bytes) u8 with
          | Core.Result.Result_Ok (len, extensions) ->
            len <=. (t13.Tls13utils.impl_Bytes__len extensions <: usize)
          | _ -> true)
      ...
      (requires
        (match trunc_len <: Core.Option.t_Option usize with
          | Core.Option.Option_Some tl ->
            tl <=.
            (t13.Tls13formats.Handshake_data.impl_HandshakeData__len client_hello <: usize)
          | _ -> true))
      (fun _ -> Prims.l_True)
...
val encrypted_extensions (e_algs: t13.Tls13crypto.t_Algorithms)
    : Prims.Pure (Core.Result.t_Result t13.Tls13formats.Handshake_data.t_HandshakeData u8)
      Prims.l_True
      (fun _ -> Prims.l_True)
...
val parse_server_certificate (certificate: t13.Tls13formats.Handshake_data.t_HandshakeData)
    : Prims.Pure (Core.Result.t_Result t13.Tls13utils.t_Bytes u8)
      Prims.l_True
      (fun _ -> Prims.l_True)

val server_certificate (e_algs: t13.Tls13crypto.t_Algorithms) (cert: t13.Tls13utils.t_Bytes)
    : Prims.Pure (Core.Result.t_Result t13.Tls13formats.Handshake_data.t_HandshakeData u8)
      Prims.l_True
      (ensures
        fun result ->
          let result:Core.Result.t_Result t13.Tls13formats.Handshake_data.t_HandshakeData u8 =
            result
          in
          match
            result <: Core.Result.t_Result t13.Tls13formats.Handshake_data.t_HandshakeData u8
          with
...
val ecdsa_signature (sv: t13.Tls13utils.t_Bytes)
    : Prims.Pure (Core.Result.t_Result t13.Tls13utils.t_Bytes u8)
      Prims.l_True
      (fun _ -> Prims.l_True)
...
val parse_ecdsa_signature (sig: t13.Tls13utils.t_Bytes)
    : Prims.Pure (Core.Result.t_Result t13.Tls13utils.t_Bytes u8)
      Prims.l_True
      (fun _ -> Prims.l_True)
...
val certificate_verify (algs: t13.Tls13crypto.t_Algorithms) (cv: t13.Tls13utils.t_Bytes)
    : Prims.Pure (Core.Result.t_Result t13.Tls13formats.Handshake_data.t_HandshakeData u8)
      Prims.l_True
      (ensures
        fun result ->
...
val parse_finished (finished: t13.Tls13formats.Handshake_data.t_HandshakeData)
    : Prims.Pure (Core.Result.t_Result t13.Tls13utils.t_Bytes u8)
      Prims.l_True
      (fun _ -> Prims.l_True)
      ...
      (ensures
        fun result ->
          ...
          | Core.Result.Result_Ok finished_msg ->
            (match
                parse_finished finished_msg <: Core.Result.t_Result t13.Tls13utils.t_Bytes u8
              with
      ...
      (ensures
        fun result ->
          ...
          | Core.Result.Result_Ok d ->
            (t13.Tls13utils.impl_Bytes__len p.t13.Tls13formats.Handshake_data._0 <: usize) <.
            mk_usize 65536 &&
            (t13.Tls13utils.impl_Bytes__len d <: usize) =.
            (mk_usize 5 +!
              (t13.Tls13utils.impl_Bytes__len p.t13.Tls13formats.Handshake_data._0 <: usize)
              <:
              usize)
...
val check_handshake_record (p: t13.Tls13utils.t_Bytes)
    : Prims.Pure
...
type t_Transcript = {
  f_hash_algorithm:t13.Tls13crypto.t_HashAlgorithm;
  f_transcript:t13.Tls13formats.Handshake_data.t_HandshakeData
}

val impl_Transcript__new (hash_algorithm: t13.Tls13crypto.t_HashAlgorithm)
...
val find_key_share (g: t13.Tls13utils.t_Bytes) (ch: t_Slice u8)
    : Prims.Pure (Core.Result.t_Result t13.Tls13utils.t_Bytes u8)
      Prims.l_True
      (fun _ -> Prims.l_True)
      (decreases
...
val server_hello
    ...
    : Prims.Pure (Core.Result.t_Result t13.Tls13formats.Handshake_data.t_HandshakeData u8)
      (requires (t13.Tls13utils.impl_Bytes__len server_random <: usize) =. mk_usize 32)
      (ensures
        fun result ->
          let result:Core.Result.t_Result t13.Tls13formats.Handshake_data.t_HandshakeData u8 =
            result
...
val client_hello
    ...
    : Prims.Pure
      (Core.Result.t_Result (t13.Tls13formats.Handshake_data.t_HandshakeData & usize) u8)
      (requires (t13.Tls13utils.impl_Bytes__len client_random <: usize) =. mk_usize 32)
      (ensures
        fun result ->
          let result:Core.Result.t_Result
