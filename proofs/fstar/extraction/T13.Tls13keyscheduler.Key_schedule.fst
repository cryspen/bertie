...
let hkdf_expand_label
  ...
  then
    ...
    let tls13_label:t13.Tls13utils.t_Bytes =
      t13.Tls13utils.impl_Bytes__concat (t13.Tls13utils.impl_Bytes__from_slice (t13.Tls13formats.v_LABEL_TLS13
              <:
              t_Slice u8)
          <:
    ...
    in
    match
      t13.Tls13utils.encode_length_u8 (t13.Tls13utils.impl_Bytes__as_raw tls13_label
          <:
          t_Slice u8)
      <:
      Core.Result.t_Result t13.Tls13utils.t_Bytes u8
    ...
    | Core.Result.Result_Ok hoist130 ->
      (match
          t13.Tls13utils.encode_length_u8 (t13.Tls13utils.impl_Bytes__as_raw context
              <:
              t_Slice u8)
          <:
          Core.Result.t_Result t13.Tls13utils.t_Bytes u8
        ...
        | Core.Result.Result_Ok hoist129 ->
          let info:t13.Tls13utils.t_Bytes =
            t13.Tls13utils.impl_Bytes__prefix (t13.Tls13utils.impl_Bytes__concat hoist130
                  hoist129
                <:
                t13.Tls13utils.t_Bytes)
              (lenb <: t_Slice u8)
...
let convert_label (label: t13.Tls13utils.t_Bytes) =
  let l:t_Slice u8 =
    (t13.Tls13utils.impl_Bytes__declassify label <: Alloc.Vec.t_Vec u8 Alloc.Alloc.t_Global).[ Core.Ops.Range.RangeFull
      <:
...
let impl: t_KeySchedule t_TLSkeyscheduler t_TLSnames =
  {
    ...
    f_labels_post
    =
    (fun (a: t_TLSnames) (b: bool) (out: Core.Result.t_Result t13.Tls13utils.t_Bytes u8) -> true);
    f_labels
    =
    ...
    (fun
        ...
        (name: t_TLSnames)
        (level: u8)
        (h: (t_TLSnames & t13.Tls13crypto.t_HashAlgorithm & u8))
        ->
        true);
    f_get_post
    =
    (fun
        ...
        (level: u8)
        (h: (t_TLSnames & t13.Tls13crypto.t_HashAlgorithm & u8))
        (out: Core.Option.t_Option t13.Tls13utils.t_Bytes)
        ->
        true);
    f_get
    =
    (fun
        ...
        (h: (t_TLSnames & t13.Tls13crypto.t_HashAlgorithm & u8))
        ->
        Core.Option.impl__map #t13.Tls13utils.t_Bytes
          #t13.Tls13utils.t_Bytes
          (Std.Collections.Hash.Map.impl_2__get #(t_TLSnames & t13.Tls13crypto.t_HashAlgorithm &
                u8)
              #t13.Tls13utils.t_Bytes
              #Std.Hash.Random.t_RandomState
              #(t_TLSnames & t13.Tls13crypto.t_HashAlgorithm & u8)
              self.f_keys
              h
            <:
            Core.Option.t_Option t13.Tls13utils.t_Bytes)
          (fun x ->
              let x:t13.Tls13utils.t_Bytes = x in
              Core.Clone.f_clone #t13.Tls13utils.t_Bytes #FStar.Tactics.Typeclasses.solve x
              <:
              t13.Tls13utils.t_Bytes));
    f_set_pre
    =
    ...
    (fun
        ...
        let tmp0, out:(Std.Collections.Hash.Map.t_HashMap
            ...
            Std.Hash.Random.t_RandomState &
          Core.Option.t_Option t13.Tls13utils.t_Bytes) =
          Std.Collections.Hash.Map.impl_2__insert #(t_TLSnames & t13.Tls13crypto.t_HashAlgorithm &
              u8)
            #t13.Tls13utils.t_Bytes
            #Std.Hash.Random.t_RandomState
            self.f_keys
        ...
        let _:Core.Option.t_Option t13.Tls13utils.t_Bytes = out in
        self);
    f_hash_pre = (fun (d: t13.Tls13utils.t_Bytes) -> true);
    f_hash_post = (fun (d: t13.Tls13utils.t_Bytes) (out: t13.Tls13utils.t_Bytes) -> true);
    f_hash
    =
    fun (d: t13.Tls13utils.t_Bytes) ->
      Core.Clone.f_clone #t13.Tls13utils.t_Bytes #FStar.Tactics.Typeclasses.solve d
  }
...
let xtr_alg (alg: t13.Tls13crypto.t_HashAlgorithm) (k1 k2: t13.Tls13utils.t_Bytes) =
  t13.Tls13crypto.hkdf_extract alg k1 k2

let xpd_alg (alg: t13.Tls13crypto.t_HashAlgorithm) (k1 label d: t13.Tls13utils.t_Bytes) =
  let kvt:Core.Option.t_Option t_Label =
    convert_label (Core.Clone.f_clone #t13.Tls13utils.t_Bytes
          #FStar.Tactics.Typeclasses.solve
          label
        <:
        t13.Tls13utils.t_Bytes)
  ...
  then t13.Tls13crypto.hmac_tag alg k1 d
  else
    hkdf_expand_label alg k1 label d (t13.Tls13crypto.impl_HashAlgorithm__hash_len alg <: usize)

let xtr (k1 k2: t_TagKey) =
  match xtr_alg k1.f_alg k1.f_val k2.f_val <: Core.Result.t_Result t13.Tls13utils.t_Bytes u8 with
  | Core.Result.Result_Ok v_val ->
    Core.Result.Result_Ok
    ({
        f_alg
        =
        Core.Clone.f_clone #t13.Tls13crypto.t_HashAlgorithm
          #FStar.Tactics.Typeclasses.solve
          k1.f_alg;
...
let xpd (k1: t_TagKey) (label d: t13.Tls13utils.t_Bytes) =
  let { f_alg = alg ; f_tag = tag ; f_val = k1 }:t_TagKey =
    Core.Clone.f_clone #t_TagKey #FStar.Tactics.Typeclasses.solve k1
  in
  match xpd_alg alg k1 label d <: Core.Result.t_Result t13.Tls13utils.t_Bytes u8 with
  | Core.Result.Result_Ok v_val ->
...
let set_by_handle (ks: t_TLSkeyscheduler) (handle: t_Handle) (key: t13.Tls13utils.t_Bytes) =
...
let zero_salt (ks: t_TLSkeyscheduler) (alg: t13.Tls13crypto.t_HashAlgorithm) =
  let handle:t_Handle =
    {
      f_alg
      =
      Core.Clone.f_clone #t13.Tls13crypto.t_HashAlgorithm #FStar.Tactics.Typeclasses.solve alg;
  ...
  let ks:t_TLSkeyscheduler =
    set_by_handle ks
      handle
      (t13.Tls13utils.impl_Bytes__zeroes (mk_usize 1) <: t13.Tls13utils.t_Bytes)
  in
  let hax_temp_output:t_Handle = handle in
  ks, hax_temp_output <: (t_TLSkeyscheduler & t_Handle)

let no_psk (ks: t_TLSkeyscheduler) (alg: t13.Tls13crypto.t_HashAlgorithm) =
  let handle:t_Handle =
    {
      f_alg
      =
  ...
  let ks:t_TLSkeyscheduler =
    set_by_handle ks
      handle
      (t13.Tls13utils.impl_Bytes__zeroes (t13.Tls13crypto.impl_HashAlgorithm__hash_len alg
            <:
            usize)
        <:
        t13.Tls13utils.t_Bytes)
...
let zero_ikm (ks: t_TLSkeyscheduler) (alg: t13.Tls13crypto.t_HashAlgorithm) =
  let handle:t_Handle =
    {
      f_alg
      =
...
let tagkey_from_handle (ks: t_TLSkeyscheduler) (handle: t_Handle) =
  match
    Core.Option.impl__ok_or #t13.Tls13utils.t_Bytes
      #u8
      (get_by_handle ks handle <: Core.Option.t_Option t13.Tls13utils.t_Bytes)
      t13.Tls13utils.v_INCORRECT_STATE
    <:
    Core.Result.t_Result t13.Tls13utils.t_Bytes u8
  with
...
let v_XPD
  ...
  | Core.Result.Result_Ok label ->
    (match
        xpd_angle n
          (Core.Clone.f_clone #t13.Tls13utils.t_Bytes #FStar.Tactics.Typeclasses.solve label
            <:
            t13.Tls13utils.t_Bytes)
          h1
          args
