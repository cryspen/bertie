...
Definition TLSnames_ES : both t_TLSnames :=
  (* TLSnames_ES  := *)
...
Definition TLSnames_EEM : both t_TLSnames :=
  (* TLSnames_EEM  := *)
...
Definition TLSnames_CET : both t_TLSnames :=
  (* TLSnames_CET  := *)
...
Definition TLSnames_Bind : both t_TLSnames :=
  (* TLSnames_Bind  := *)
...
Definition TLSnames_Binder : both t_TLSnames :=
  (* TLSnames_Binder  := *)
...
Definition TLSnames_HS : both t_TLSnames :=
  (* TLSnames_HS  := *)
...
Definition TLSnames_SHT : both t_TLSnames :=
  (* TLSnames_SHT  := *)
...
Definition TLSnames_CHT : both t_TLSnames :=
  (* TLSnames_CHT  := *)
...
Definition TLSnames_HSalt : both t_TLSnames :=
  (* TLSnames_HSalt  := *)
...
Definition TLSnames_AS : both t_TLSnames :=
  (* TLSnames_AS  := *)
...
Definition TLSnames_RM : both t_TLSnames :=
  (* TLSnames_RM  := *)
...
Definition TLSnames_CAT : both t_TLSnames :=
  (* TLSnames_CAT  := *)
...
Definition TLSnames_SAT : both t_TLSnames :=
  (* TLSnames_SAT  := *)
...
Definition TLSnames_EAM : both t_TLSnames :=
  (* TLSnames_EAM  := *)
...
Definition TLSnames_PSK : both t_TLSnames :=
  (* TLSnames_PSK  := *)
...
(* Definition t_TLSnames_cast_to_repr (x : both t_TLSnames) : both uint_size := *)
...
Definition label_to_bytes (label : both t_Label) : both t_Bytes.
  (* label_to_bytes label  := *)
    refine (matchb label with
    | Label_EXT_BINDER_e__case  =>
...
(* #[global] (* Program *) Instance t_TLSkeyscheduler_t_KeySchedule : t_KeySchedule (v_N := 'nat) t_TLSkeyscheduler t_TLSnames. (* := *) *)
(*   refine (let f_prnt_n := fun  (a : both t_TLSnames) => matchb a with *)
(*   | TLSnames_ES_case  => *)
...
(*   let f_hash := fun  (d : both t_Bytes) => f_clone d : both t_Bytes in _). *)
(*   {| f_prnt_n := (@f_prnt_n); *)
(*   f_labels := (@f_labels); *)
(*   f_get := (@f_get); *)
(*   f_set := (@f_set); *)
...
Definition f_alg (s : both t_TagKey) : both t_HashAlgorithm :=
  (* f_alg s  := *)
    bind_both s (fun x =>
...
(* Instance t_TagKey_settable : Settable _ := settable! prod_both < f_tag  >. *)
Definition f_alg_p (s : t_TagKey) : t_HashAlgorithm :=
  (* f_alg s  := *)
    fst (fst s) : t_HashAlgorithm.
Fail Next Obligation.
Definition f_tag_p (s : t_TagKey) : t_TLSnames :=
  (* f_tag s  := *)
  snd (fst s) : t_TLSnames.
Fail Next Obligation.
Definition f_val_p (s : t_TagKey) : t_Bytes :=
  (* f_val s  := *)
    snd s : t_Bytes.
Fail Next Obligation.

Definition Build_t_TagKey {f_alg : both t_HashAlgorithm} {f_tag : both t_TLSnames} {f_val : both t_Bytes} : both (t_TagKey) := prod_b (f_alg , f_tag , f_val).
...
Equations Build_t_Handle {f_name : both t_TLSnames} {f_Handle_alg : both t_HashAlgorithm} {f_level : both int8} : both (t_Handle) :=
  Build_t_Handle  :=
    prod_b (f_name,f_Handle_alg,f_level).
