...
Section Core.
  ...
  (*              (DH_package k ∘ Nk_package 0 k (leq0n k) ∘ (L_package k DH Z)) *)
  (*           ) *)
  ...
  (* Obligation Tactic := (* try timeout 8 *) idtac. *)
  (* Program Definition Gcore_real (d k : nat) b H_lt : *)
  (*   package (L_K :|: L_L) *)
  (*           [interface] *)
  (*     (* ((GET_XPD :|: SET_XPD) *) *)
  (*     (*     :|: DH_Set_interface *) *)
  ...
  (*   (*    GET_o_star_ℓ d) *) *)
  (*   := *)
  (*   {package (Ks d k (ltnW H_lt) O_star false erefl ∘ Ls k O_star (fun _ => F) erefl) ∘ XPD_DH_XTR d k b (fun _ => b) H_lt}. *)
  (* Final Obligation. *)
  (*   intros. *)
  (*   rewrite <- fsetUid. *)
  (*   eapply valid_link. *)
  ...
  (* HB.instance Definition _ : ordType chName := *)
  (*   _. *)
  ...
  Proof.
    ...
    all: try now (intros n x n0 u1 u2 ; rewrite !in_fset !(mem_seq1 _ _) => /eqP ? /eqP ?).
    all: try now (intros n x u1 u2 ; rewrite !in_fset !(mem_seq1 _ _) => /eqP ? /eqP ?).
  ...
  Program Definition G_core_Hash (d k : nat) (H_lt : (d < k)%nat) :
    package (L_K :|: L_L)
            ...
            (XPD_n d k
               ...
               :|: GET_n O_star d k) :=
    {package
               (par (G_check_XTR_XPD d k H_lt (fun _ => false))
                  (par
                     (G_dh d k (ltnW H_lt))
               ...
               (KeysAndHash d k H_lt (fun _ _ => false) (fun _ => Z))
    ...
    1:{
      ...
      2:{
        ...
        }
        {
          (* rewrite <- fsetUid. *)
          (* rewrite fsetUA. *)
          (* rewrite <- (fsetUC (interface_hierarchy_foreach _ O_star _)). *)
          eapply valid_par_upto.
          2: apply pack_valid.
          2: apply pack_valid.
          2: solve_in_fset.
  ...
  Program Definition G_core_hyb_ℓ (d k : nat) (H_lt : (d < k)%nat) (i : nat) :
    ...
    {package
               ...
               (par (par (Ks d k (ltnW H_lt) all_names (fun ℓ name =>
                       ...
                       (K_package k PSK d.+1 H_lt (i == d) ∘ L_package k PSK D)) (Hash true))
    }.
  Solve Obligations with intros ; solve_idents.
  Solve Obligations with intros ; now intros ? ? ? ; rewrite !in_fset !(mem_seq1 _ _) => /eqP ? /eqP ?.
  Solve Obligations with easy.
