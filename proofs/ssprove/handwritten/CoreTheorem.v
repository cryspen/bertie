...
Section CoreTheorem.
  ...
  Definition xpd_xpr_approximation
    ...
    package (L_K :|: L_L)
      ...
      (XPD_n d k :|: XTR_n d k).
  Proof.
    refine ({package par (XPD_ d k H_lt) (XTR_ d k (ltnW H_lt))}).
    unfold XPD_, XTR_.
    unfold pack.
  ...
  Proof.
    ...
    refine ({package (par
       ...
       (XTR_packages d k (ltnW (m:=d) (n:=k) H_lt)
  ...
  Definition core (d k : nat) (H_lt : (d < k)%nat) :
    package fset0
    ...
    forall (LA : {fset Location}) (A : raw_package),
      ...
      (AdvantageE
       ...
       <= sumR_l [R_cr; (R_Z f_hash); R_D] (fun R => Advantage (Gacr f_hash) (A ∘ R))
         +maxR (fun i => Advantage Gsodh (Ai A i ∘ R_sodh) + AdvantageE (Gcore_sodh d k false) (Gcore_ideal d k (ltnW H_lt) Score) (Ai A i))
      )%R.
  Proof.
    intros.
    unfold sumR_l.
  ...
  Lemma equation20_lhs :
    ...
    forall (LA : {fset Location}) (A : raw_package),
      ValidPackage LA (KS_interface d k) A_export A →
      (AdvantageE (Gcore_sodh d k false) (Gcore_hyb d 0) (Ai A i) = 0)%R.
  Proof.
    intros.
  Admitted.
  ...
  Lemma equation20_eq :
    ...
    forall (LA : {fset Location}) (A : raw_package),
      ValidPackage LA (KS_interface d k) A_export A →
      (AdvantageE (Gcore_sodh d k false) (Gcore_ideal d k H_lt Score) (Ai A i)
