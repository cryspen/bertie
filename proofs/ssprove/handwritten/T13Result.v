...
From t13Extraction Require Import t13_Tls13keyscheduler_Key_schedule.

(* ./extraction/Fixes.v *)
(* ./extraction/t13_Tls13utils.v *)
(* ./extraction/t13_Tls13formats_Handshake_data.v *)
(* ./extraction/t13_Tls13crypto.v *)
(* ./extraction/t13_Tls13formats.v *)
(* ./extraction/t13_Tls13keyscheduler_Key_schedule.v *)
...
Section t13KeySchedule.
  Definition t13_PrntN : name -> (* code fset0 fset0 *) (chProd chName chName) :=
    (fun n =>
  ...
  Lemma t13_PrntN_correct : ∀ n : name, PrntN n == t13_PrntN n.
  Proof.
    intros.
    apply /eqP.
    destruct n ; unfold PrntN ; unfold t13_PrntN ; now try rewrite let_both_equation_1.
  Qed.

  Program Definition t13_Labels : name -> bool -> code fset0 fset0 chLabel :=
    fun n b => {code
  ...
  Definition t13_O_star : list name :=
    [EEM; CET; BIND; BINDER; SHT; CHT; HSALT; RM; CAT; SAT; EAM; PSK; ZERO_SALT; ESALT; DH; ZERO_IKM].

  Program Definition t13_xpd : chKey -> (chLabel * bitvec) -> code fset0 fset0 chKey :=
  ...
  Definition t13_xtr : chKey -> chKey -> code fset0 fset0 chKey.
  Proof.
    intros.
  ...
  Definition t13_xtr_angle : name -> chHandle -> chHandle -> code fset0 fset0 chHandle.
    intros n h1 h2.
    ...
    refine (
      {code
         is_state (
           letb x := t13_Tls13keyscheduler_Key_schedule.xtr_angle (name_to_t_TLSname (H_not_bot := _) n) (ret_both (chHandle_to_t_Handle h1 _)) (ret_both (chHandle_to_t_Handle h2 _)) in
             matchb x with
  ...
  Definition t13_xpd_angle : name -> chLabel -> chHandle -> bitvec -> code fset0 fset0 chHandle.
  Admitted.
  Definition t13_PrntIdx : name -> forall (ℓ : bitvec), code fset0 [interface] (chProd chName chName). Admitted.
  Definition t13_ord : chGroup → nat. Admitted.
  Definition t13_E : nat -> nat. Admitted.

  Definition t13_L_K : {fset Location}. Admitted.
  Definition t13_K_table : chHandle -> nat. Admitted.
  Definition t13_in_K_table : forall x, ('option chK_table; t13_K_table x) \in t13_L_K. Admitted.

  Definition t13_L_L : {fset Location}. Admitted.
  Definition t13_L_table : chHandle -> nat. Admitted.
  Definition t13_in_L_table : forall x, ('option chL_table; t13_L_table x) \in t13_L_L. Admitted.

  Definition t13_L_M : {fset Location}. Admitted.
  Definition t13_M : chHandle -> nat. Admitted.
  Definition t13_H : name → ∀ s : chHandle, ('option ('fin #|fin_handle|); t13_M s) \in t13_L_M. Admitted.

  Definition t13_DHGEN_function : chGroup -> code fset0 fset0 chGroup. Admitted.
  Definition t13_DHEXP_function : chGroup -> chGroup -> code fset0 fset0 chHandle. Admitted.

End t13KeySchedule.

Instance t13KeySchedule (d : nat) : Dependencies :=
  {
    TLSPrntN := t13_PrntN;
    TLSPrntN_is_PrntN := t13_PrntN_correct ;
    Labels := t13_Labels ;

    xpd := t13_xpd ;
    xtr := t13_xtr ;
    xtr_angle := t13_xtr_angle ;
    xpd_angle := t13_xpd_angle ;
    PrntIdx := t13_PrntIdx ;
    ord := t13_ord ;
    E := t13_E ;
    ...
    L_L := t13_L_L ;
    L_table := t13_L_table  ;
    in_L_table := t13_in_L_table ;

    L_K := t13_L_K ;
    K_table := t13_K_table ;
    in_K_table := t13_in_K_table ;
    ...
    DHGEN_function := t13_DHGEN_function ;
    DHEXP_function := t13_DHEXP_function ;
  }.

Definition t13KeyScheduleCoreTheorem (d k : nat) (H_lt : (d < k)%nat) :=
  core_theorem (DepInstance := t13KeySchedule d) d k H_lt.
