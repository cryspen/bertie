...
pub fn hkdf_expand_label(
    ...
    if len >= 65536 {
        ...
        match hkdf_expand(ha.clone(), k, &info, len as usize) {
            Ok(k) => Res::<Key>::Ok(k),
...
pub fn derive_binder_key(ha: &HashAlgorithm, k: &Key) -> Res<MacKey> {
    let early_secret = hkdf_extract(ha.clone(), k, &zero_key(ha))?;
...
pub fn derive_hk_ms(
    ...
    let early_secret = hkdf_extract(ha.clone(), &psk, &zero_key(ha))?;
    ...
    let handshake_secret = hkdf_extract(ha.clone(), gxy, &derived_secret)?;
    ...
    let master_secret = hkdf_extract(ha.clone(), &zero_key(ha), &master_secret_)?;
    Ok((
        client_write_key_iv,
        server_write_key_iv,
        client_finished_key,
...
pub struct TranscriptServerCertificateVerify(
    pub HashAlgorithm,
    pub bool,
    pub HandshakeData,
    pub Digest,
...
pub fn transcript_empty(ha: &HashAlgorithm) -> Transcript {
    Transcript(ha.clone(),HandshakeData(empty()))
}
...
pub fn transcript_server_hello(
    tx: TranscriptClientHello,
    sh: &HandshakeData,
) -> Res<TranscriptServerHello> {
...
pub fn transcript_server_certificate(
    tx: TranscriptServerHello,
    ee: &HandshakeData,
    sc: &HandshakeData,
) -> Res<TranscriptServerCertificate> {
...
pub fn transcript_server_certificate_verify(
    tx: TranscriptServerCertificate,
    cv: &HandshakeData,
) -> Res<TranscriptServerCertificateVerify> {
    let TranscriptServerCertificate(ha, psk_mode, transcript, _) = tx;
    if psk_mode {
        Err(psk_mode_mismatch)
    } else {
        let (transcript, th) = transcript_add(ha, transcript, cv)?;
        Ok(TranscriptServerCertificateVerify(
            ha, psk_mode, transcript, th,
        ))
    }
}

pub fn transcript_skip_server_certificate_verify(
    tx: TranscriptServerHello,
    ee: &HandshakeData,
) -> Res<TranscriptServerCertificateVerify> {
    let TranscriptServerHello(ha, psk_mode, transcript, _) = tx;
...
pub fn transcript_server_finished(
    tx: TranscriptServerCertificateVerify,
    sf: &HandshakeData,
) -> Res<TranscriptServerFinished> {
...
pub fn transcript_client_finished(
    tx: TranscriptServerFinished,
    cf: &HandshakeData,
) -> Res<TranscriptClientFinished> {
...
pub struct ServerPostClientHello(Random, Random, Algorithms, Bytes, Bytes, SignatureKey, Bytes, Key, Option<PSK>, Transcript);
    ...
    if ent.len() < 32 + dh_priv_len(ks) {
        ...
        match (*psk_mode, psk, trunc_len) {
            (true, Some(k), _) => {
                ...
                let binder = hmac_tag(ha, &mk, &bytes(&th_trunc))?;
                ...
                if *zero_rtt {
                    ...
                    return Ok((nch, None, ClientPostClientHello(cr, algs0, x, psk,tx_ch)))
                }
            },
            (false, None, 0) => {
                let tx_ch = transcript_add1(tx,&ch);
                return Ok((ch,None,ClientPostClientHello(cr,algs0,x,psk,tx_ch)))
            },
        ...
        match (psk_mode, psko, bindero) {
            (true, Some(k), Some(binder)) => {
                ...
                if *zero_rtt {
                    ...
                    return Ok((cipher0,ServerPostClientHello(cr, sr, algs, sid, cert, sigk, gy, gxy, Some(k), tx)))
                }
                else {
                    return Ok((None,ServerPostClientHello(cr, sr, algs, sid, cert, sigk, gy, gxy, Some(k), tx)))
                }
            },
            (false, None, None) => return Ok((None,ServerPostClientHello(cr, sr, algs, sid, cert, sigk, gy, gxy, None, tx))),
            _ => return Err(psk_mode_mismatch),
        }
    }
}
...
pub fn get_server_hello(
    ...
    let ServerPostClientHello(cr, sr, algs, sid, cert, sigk, gy, gxy, psk, tx) = st;
    let Algorithms(ha, ae, sa, gn, psk_mode, zero_rtt) = &algs;
